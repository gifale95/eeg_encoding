...
def perform_regression(args, ch_names, times, X_train, X_test, X_ilsvrc2012_val,
	...
	and training EEG data (Y), and use the trained weights to predict the EEG
	...
	pred_train_within = np.reshape(reg_within.predict(X_train),
		(X_train.shape[0],eeg_shape[1], eeg_shape[2]))
	pred_test_within = np.reshape(reg_within.predict(X_test), (X_test.shape[0],
		eeg_shape[1],eeg_shape[2]))
	pred_ilsvrc2012_val_within = np.reshape(reg_within.predict(
		X_ilsvrc2012_val), (X_ilsvrc2012_val.shape[0],eeg_shape[1],
		eeg_shape[2]))
	pred_ilsvrc2012_test_within = np.reshape(reg_within.predict(
	...
	pred_train_between = np.reshape(reg_between.predict(X_train),
		(X_train.shape[0],eeg_shape[1],eeg_shape[2]))
	pred_test_between = np.reshape(reg_between.predict(X_test),
	...
	save_dir = os.path.join(args.project_dir, 'results', 'sub-'+
		format(args.sub,'02'), 'predicted_eeg_data', 'dnn-'+args.dnn)
	if not os.path.exists(save_dir):
		os.makedirs(save_dir)
	# Training data
	data_dict = {
		'predicted_within_data': pred_train_within,
		'predicted_between_data': pred_train_between,
		'ch_names': ch_names,
		'times': times
		}
	file_name = 'predicted_eeg_training.npy'
	np.save(os.path.join(save_dir, file_name), data_dict)
	# Test data
	data_dict = {
		'predicted_within_data': pred_test_within,
		'predicted_between_data': pred_test_between,
		'ch_names': ch_names,
		'times': times
		}
	file_name = 'predicted_eeg_test.npy'
	np.save(os.path.join(save_dir, file_name), data_dict)
	# ILSVRC-2012 validation data
	data_dict = {
		'predicted_data_within': pred_ilsvrc2012_val_within,
		'ch_names': ch_names,
		'times': times
		}
	file_name = 'predicted_eeg_ilsvrc2012_val.npy'
	np.save(os.path.join(save_dir, file_name), data_dict)
	# ILSVRC-2012 test data
	data_dict = {
		'predicted_data_within': pred_ilsvrc2012_test_within,
		'ch_names': ch_names,
		'times': times
		}
	file_name = 'predicted_eeg_ilsvrc2012_test.npy'
	np.save(os.path.join(save_dir, file_name), data_dict)
